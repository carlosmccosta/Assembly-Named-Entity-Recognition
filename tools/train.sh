...
java -mx${java_max_memory} -cp ${javaclasspath} edu.stanford.nlp.ie.crf.CRFClassifier -prop "${properties_file}" -trainFile "${input_file}" -gazette "${gazette_file}" -serializeTo "${output_file_basename}-ner-model.ser.gz" > "${output_file_basename}-training-output.txt" 2>&1
